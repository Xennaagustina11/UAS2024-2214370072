...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="mb-4">
                <img src="/storage/{{ $article->gambar }}" alt="{{ $article->judul }}" class="rounded-lg w-full">
            </div>

            <div>
                <h1 class="text-4xl font-bold mb-4">{{ $article->judul }}</h1>
                <p class="text-gray-600 text-sm mb-2">Kategori: {{ $article->kategori }}</p>
                <p class="text-gray-600 text-sm mb-6">Ditulis oleh {{ $article->user->name }}</p>
